...
const storage = {
  ...
  getCurrentMonthID() { return localStorage.getItem("mesIDActual") || ""; },
  setCurrentMonthID(id) { localStorage.setItem("mesIDActual", id); }
};
...
const formatearIDMes = (idKey) => {
  if (!idKey) {
    return new Date().toLocaleDateString("es-MX", { year: 'numeric', month: 'long' });
  }
...
const ui = {
  ...
  renderSummary() {
    ...
    $("#fechaActual").textContent = formatearIDMes(state.currentMonthID);

    // Ajuste dinámico inteligente del botón principal de acción
    const btnFinalizar = $("#btnFinalizarMes");
    if (btnFinalizar) {
      if (state.currentMonthID) {
        btnFinalizar.textContent = "Terminar de Editar ↩️";
        btnFinalizar.style.backgroundColor = "#dc2626"; 
      } else {
        btnFinalizar.textContent = "Finalizar Mes";
  ...
  closeSummary() { $("#modal-resumen").style.none; $("#modal-resumen").style.display = "none"; },
  ...
  viewMonthDetails(mes) {
    ...
    const contenedor = $("#lista-meses");
    contenedor.innerHTML = `
      <button id="btnCargarEditor" style="background:#007bff; color:#fff; border:2px solid #fff; padding:12px; margin-bottom:12px; border-radius:5px; font-weight:bold; width:100%; font-size:15px; box-sizing:border-box;">Editar mes</button>
    ...
    $("#btnCargarEditor").addEventListener("click", () => {
      if (!confirm(`¿Deseas editar "${etiquetaLegible}"?`)) return;
      state.active = mes.viajes;
      state.currentMonthID = mes.id;
      storage.save(state.active);
      storage.setCurrentMonthID(state.currentMonthID);
    ...
    $("#btnBorrarMes").addEventListener("click", async () => {
      ...
      try {
        ...
        if (state.currentMonthID === mes.id) {
          state.active = [];
          state.currentMonthID = "";
          storage.clear();
          storage.setCurrentMonthID("");
          ui.renderTable();
          ui.renderSummary();
        }
        alert("Mes eliminado.");
  ...
  resetForm() {
    ...
    if (state.currentMonthID) {
      const [year, monthNum] = state.currentMonthID.split("-");
      $("#fecha").value = `${year}-${monthNum}-01`;
    }
  },
...
const actions = {
  async save() { 
    storage.save(state.active);
    if (state.currentMonthID) {
      await dbStorage.saveMonth(state.currentMonthID, state.active);
    }
  },

  async addOrUpdateFromForm() {
    const f = $("#fecha").value;
    ...
    if (state.currentMonthID) {
      const idFormulario = `${v.anio}-${String(m).padStart(2, "0")}`;
      if (idFormulario !== state.currentMonthID) {
        alert(`Error: Estás editando el historial de ${formatearIDMes(state.currentMonthID)}. No puedes añadir un viaje con fecha de otro mes.`);
        return;
      }
    }
    ...
    if (viajesMismoDia.length >= 2) {
      alert(`Operación inválida: Ya existen 2 viajes registrados para el día ${v.dia} de ${v.mes}.`);
      return;
    }

    if (viajesMismoDia.length === 1 && viajesMismoDia[0].turno === v.turno) {
      alert(`Conflicto de turno: Ya hay un viaje registrado en el "${v.turno}" para el día ${v.dia}.`);
      return;
    }
  ...
  async finalizeMonth() {
    ...
    // Si estamos editando el pasado, el botón funciona para SALIR del modo edición
    if (state.currentMonthID) {
      if (!confirm("¿Terminar de editar?")) return;
      state.active = [];
      state.currentMonthID = "";
      storage.clear();
      storage.setCurrentMonthID("");
    ...
    try {
      ...
      ui.renderTable();
      ui.renderSummary();
      alert(`Mes finalizado con éxito: ${formatearIDMes(idKey)}`);
    } catch (error) {
      console.error(error);
    }
  }
...
function init() {
  ...
  state.currentMonthID = storage.getCurrentMonthID();
  sorters.byDate(state.active);
  ui.renderTable();
  ui.renderSummary();
  bindEvents();
